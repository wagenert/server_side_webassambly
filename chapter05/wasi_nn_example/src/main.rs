...
fn main() {
    ...
    let class_labels: Vec<String> = labels_data.lines().map(|line| line.to_string()).collect();
    let data = image_to_tensor(&image_path, 224, 224);
    let dimensions = vec![1, 3, 224, 224];
    exec_context
        .set_input(0, TensorType::F32, &dimensions, &data)
        .unwrap();
    exec_context.compute().unwrap();
    let mut out_buffer = vec![0f32; 1001];
    let output_data = exec_context.get_output(0, &mut out_buffer).unwrap();
    let output_array = Array::from_vec(out_buffer);
